...
{#if loading}
	<div class="flex items-center justify-center h-full">
		<div class="flex flex-col items-center gap-4">
			<div
				class="w-10 h-10 border-2 border-[#1DB954] border-t-transparent rounded-full animate-spin"
			></div>
...
{:else}
	<div class="flex flex-col h-full">
		<!-- Profile Header -->
		<div
			class="relative px-4 sm:px-8 pt-5 sm:pt-8 pb-4 sm:pb-6 bg-gradient-to-b from-[#1DB954]/15 via-[#1DB954]/5 to-transparent"
		>
			<div class="flex items-center gap-4 sm:gap-5">
				...
				{:else}
					<div
						class="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-zinc-800 flex items-center justify-center ring-2 ring-[#1DB954]/30 shadow-xl"
					>
						<svg
							class="w-8 h-8 sm:w-10 sm:h-10 text-zinc-600"
							fill="currentColor"
							viewBox="0 0 24 24"
						>
							<path
		...
		<div class="flex flex-col sm:flex-row flex-1 min-h-0">
			...
			<div
				class="sm:w-56 shrink-0 border-b sm:border-b-0 sm:border-r border-zinc-800/50 flex flex-col"
			>
				<div class="px-4 py-3">
					<h3 class="text-[11px] font-semibold text-zinc-500 uppercase tracking-wider">
						Playlists
					</h3>
				</div>
				...
				<div class="overflow-y-auto max-h-40 sm:max-h-none sm:flex-1 spotify-scroll">
					{#each playlists as playlist (playlist.id)}
						<button
							class="w-full flex items-center gap-3 px-4 py-2.5 text-left transition-all duration-150 {activePlaylistId ===
							playlist.id
						...
						>
							...
							<div class="min-w-0 flex-1">
								<p
									class="text-sm font-medium truncate {activePlaylistId === playlist.id
										? 'text-[#1DB954]'
										: 'text-zinc-200'}"
								>
									{playlist.name}
			...
			<div class="flex-1 flex flex-col min-w-0">
				{#if activePlaylist}
					<!-- Playlist Header -->
					<div
						class="flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-3 sm:py-4 border-b border-zinc-800/50"
					>
						<img
					...
					<div class="flex-1 overflow-y-auto spotify-scroll">
						<!-- Column Headers -->
						<div
							class="flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-2 border-b border-zinc-800/30 text-[11px] font-medium text-zinc-600 uppercase tracking-wider sticky top-0 bg-zinc-900/95 backdrop-blur-sm z-10"
						>
						...
						{#each filteredTracks() as track, i (track.title + i)}
							<div
								class="group flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-2.5 hover:bg-zinc-800/40 transition-colors"
							>
								<!-- Track Number -->
								<span
									class="w-6 sm:w-8 text-center text-sm text-zinc-600 group-hover:text-zinc-400 shrink-0 tabular-nums"
								>
									{i + 1}
								</span>
								...
								<div class="flex-1 min-w-0">
									<p class="text-sm text-zinc-200 group-hover:text-white truncate">
										...
										{#if track.isExplicit}
											<span
												class="inline-flex items-center justify-center w-4 h-4 text-[9px] font-bold bg-zinc-700 text-zinc-400 rounded ml-1.5 align-middle"
												>E</span
											>
										{/if}
									</p>
									<p class="text-xs text-zinc-500 group-hover:text-zinc-400 truncate">
										{track.artist}
									</p>
								</div>
								...
								<!-- Duration -->
								<span
									class="w-12 text-right text-xs text-zinc-600 group-hover:text-zinc-400 tabular-nums shrink-0"
								>
						...
						{#if filteredTracks().length === 0 && search}
							<div class="flex flex-col items-center justify-center py-16 text-zinc-500">
								<svg
									class="w-8 h-8 mb-3 text-zinc-700"
									fill="none"
									stroke="currentColor"
									stroke-width="1.5"
									viewBox="0 0 24 24"
